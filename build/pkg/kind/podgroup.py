...
podgroup: Blueprint = Blueprint("podgroup", __name__, url_prefix="/podgroup")


@podgroup.route("/", methods=["POST"])
def validate_webhook():
    request_info = dict()
    if isinstance(request.json, Dict):
        request_info = request.json
    if request_info["request"]["operation"] == CREATE:
        podgroup.logger.debug(request_info)
...
@podgroup.route("/mutate", methods=["POST"])
def mutate_webhook():
    allowed = True
    request_info = dict()
    if isinstance(request.json, Dict):
...
def create_validate(request_object: Dict) -> Tuple[bool, str]:
    podgroup.logger.debug("create_validate")
    status = ""
    allowed = True
    try:
...
def update_validate(request_object: Dict) -> Tuple[bool, str]:
    podgroup.logger.debug("update_validate")
    status = ""
    allowed = True
...
def create_mutate(request_object):
    podgroup.logger.debug("create_mutate")
...
def update_mutate(request_object):
    podgroup.logger.debug("update_mutate")
    podgroup.logger.debug(request_object)
